...
See [New] for the uniqueness and ordering guarantees. Uniqueness is guaranteed
per process only; see DESIGN.md's Uniqueness section.
...
Security note: an ID carries only 20 bits of randomness alongside values derived
from the clock. Do not use them where unguessability matters, such as session
tokens, API keys, or password reset codes.

The API borrows from github.com/rs/xid. See DESIGN.md for the byte layout and
its history, and NOTICES for third-party license texts.
*/
package kid
...
func NewWithTime(t time.Time) (id ID, err error) {
	...
	if milli < 0 || milli >= 1<<48 {
		return ID{}, ErrTimestampOutOfRange
	}
...
// UnmarshalBinary copies data into id. On error, id is set to the zero
// value and ErrInvalidID is returned. UnmarshalBinary must be able to
// decode the form generated by MarshalBinary, so it accepts only exactly
// rawLen (10) bytes, same as FromBytes.
...
// id is set to the zero value and ErrInvalidID is returned.
...
// MarshalJSON encodes id as a quoted string, or null for the zero value.
// Every ID encodes, so error is always nil.
...
// UnmarshalJSON accepts null (the zero value) or a quoted 16-character
// kid encoding.
//
